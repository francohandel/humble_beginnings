...
payroll.title("Book Management Systems")
...
def emprec(event):
    ...
    txtbkid.insert(END, ed[0])
    txtbkname.delete(0, END)
    txtbkname.insert(END, ed[1])
    #txtisbn.delete(0, END)
    #txtisbn.insert(END, ed[])
    txtauth.delete(0, END)
    txtauth.insert(END, ed[2])
    txtPrice.delete(0, END)
    txtPrice.insert(END, ed[3])
    txtPress.delete(0, END)
    txtPress.insert(END, ed[4])
    txtdate.delete(0, END)
    txtdate.insert(END, ed[5])
...
def addData():
    if len(bookname.get()) != 0:
        database.addbkrec(ISBN.get(), bookname.get(),author.get(), price.get(), press.get(), datei.get())
        emplist.delete(0, END)
        emplist.insert(END, (ISBN.get(), bookname.get(), author.get(), price.get(), press.get(), datei.get()))
...
def SearchData():
    emplist.delete(0, END)
    for row in database.Searchrec(ISBN.get(), bookname.get(), author.get(), price.get(), press.get(), datei.get()):
        emplist.insert(END, row, str(""))
...
def update():
    ...
    if (len(ISBN.get()) != 0):
        database.UpdateData(ISBN.get(),bookname.get(), author.get(), price.get(), press.get(), datei.get())
...
lblbkid = Label(LF, font=('arial', 12, 'bold'), text="ISBN", fg="Steel blue", bd=10, anchor="w")
...
btnExit = Button(RF, padx=8, pady=8, fg="black", font=('arial', 12, 'bold'), width=20,
                 text="Exit", bg="sky blue", command=iExit).grid(row=5, column=0)

btnsearch = Button(RF, padx=8, pady=8, fg="black", font=('arial', 12, 'bold'), width=20,
                 text="search", bg="sky blue", command=SearchData).grid(row=6, column=0)               
